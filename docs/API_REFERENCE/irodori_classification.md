...
- **Description:**  
  Split `HyperTable` data into training and testing sets.
...
- **Parameters:**  
  - `X_train`: Training feature matrix  
  - `X_test`: Test feature matrix  
  - `n_components (int)`: Number of principal components (default `10`)  
...
- **Parameters:**  
  ...
  - `method (str)`: Supported methods: `'svm'`, `'rf'`, `'knn'`, `'mlp'`  
...
- **Description:**  
  Evaluate classifier performance with accuracy, classification report, and confusion matrix.
...
- **Description:**  
  Plot top-k feature importances (Random Forest only).
...
- **Description:**  
  End-to-end pipeline: split → scale → PCA → train → evaluate.

- **Returns:**  
  - `Dictionary containing classifier, scaler, pca, accuracy, classification_report, confusion_matrix`  
...
- **Description:**  
  Persist or load trained classifier using joblib.
...
- **Description:**  
  Returns a table with precision, recall, F1-score, and support per class.
...
- **Description:**  
  Compute top-k accuracy for classifiers with `predict_proba`.
...
- **Description:**  
  Plot reliability diagram and return Brier score.
...
- **Description:**  
  Project feature space into 2D using t-SNE for visualization.
...
- **Description:**  
  Project feature space into 2D using UMAP for visualization.
...
- **Description:**  
  Plot top-k features using permutation importance.
...
- **Description:**  
  Train ensemble models: Voting or Stacking classifiers.
...
- **Description:**  
  Wrap a base classifier for multiclass strategies.
...
- **Returns:**  
  - `Best threshold and corresponding score`  
...
- **Returns:**  
  - `pd.DataFrame` with predictions, true labels, and probabilities  
